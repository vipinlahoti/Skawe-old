...
Users.canView = function (user, document) {
  ...
  const status = _.findWhere(Skawe.statuses, {value: document.status}).label;
...
Users.canSubmitField = function (user, field) {
  return user && field.insertableBy && field.insertableBy(user);
};
...
Users.canEditField = function (user, field, document) {
  return user && field.editableBy && field.editableBy(user, document);
};
