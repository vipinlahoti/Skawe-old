 ...
 */
Skawe.callbacks.add = function (hook, callback) {

  // if callback array doesn't exist yet, initialize it
  if (typeof Skawe.callbacks[hook] === 'undefined') {
    Skawe.callbacks[hook] = [];
  }

  Skawe.callbacks[hook].push(callback);
};
 ...
 */
Skawe.callbacks.remove = function (hookName, callbackName) {
  Skawe.callbacks[hookName] = _.reject(Skawe.callbacks[hookName], function (callback) {
 ...
 * @param {Any} args - Other arguments will be passed to each successive iteration
 */
Skawe.callbacks.runAsync = function () {

  // the first argument is the name of the hook
  var hook = arguments[0];
  // successive arguments are passed to each iteration
  var args = Array.prototype.slice.call(arguments).slice(1);
  var callbacks = Skawe.callbacks[hook];
